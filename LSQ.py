...
def decompose_weights(w_q, scales):
    ...
    for i in range(len(quant_values)):
        ...
        pos_comp = torch.zeros(n_bit, device=device)
        neg_comp = torch.zeros(n_bit, device=device)

        for j in range(n_bit):
            if coefficients[i, j] == 1:
                pos_comp[j] = scales[j].item()
            elif coefficients[i, j] == -1:
                neg_comp[j] = scales[j].item()

        mask = (torch.abs(w_q - q_val) < 1e-6)  # Small epsilon for float comparison
        if mask.any():
            wp[mask] = pos_comp.sum()
            wn[mask] = neg_comp.sum()
