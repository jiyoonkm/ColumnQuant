...
class LsqPsum(nn.Module):
    def __init__(self, bit, num_sigma, per_channel, oc):
        ...
        # # array-wise
        # if self.per_channel:
        #     self.sf = nn.Parameter(torch.ones(1, self.oc, 1, 1)*4/num_levels)
        # else:
        #     self.sf = nn.Parameter(torch.tensor([4/num_levels]))
    ...
    def forward(self, x):
        # s_grad_scale = 1.0 / ((self.thd_pos * x.numel()) ** 0.5)                        # LSQ paper v1
        s_grad_scale = 1e-1                                                             # LSQ paper v2: activation
        s_scale = grad_scale(self.sf, s_grad_scale)
